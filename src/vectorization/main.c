...
void print_matrix(double* restrict flat_matrix) {
...
bool is_matrix_nan(double* restrict flat_matrix) {
    for(size_t row = 0; row < 8; ++row) {
        for(size_t col = 0; col < NODES; ++col) {
...
void set_row_values(
    double* restrict x, double* restrict rho, double* restrict Vx, double* restrict Vy,
    double* restrict Vz, double* restrict p, double* restrict Bx, double* restrict By,
    double* restrict Bz) {
    double rho1 = 1.0;
    double Vx1 = 0.0;
    double Vy1 = 0.0;
...
double* getU(
    ...
    double* restrict rho, double* restrict p,
...
double* getLU(double* restrict U, double* restrict x) {
...
double* getNU(double* restrict U, double* restrict x){
    double* LU = getLU(U, x);
