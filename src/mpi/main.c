...
#define LOCAL_U_SIZE(data_size) data_size * ROWS

void print_matrix(double* flat_matrix, int size) {
    for(size_t row = 0; row < 8; ++row) {
        for(size_t col = 0; col < size; ++col) {
            printf("%.4f\t", flat_matrix[row * size + col]);
        }
        printf("\n");
    }
}
...
int get_chunk_size(int total_size, int processors, int process_id) {
    return (total_size / processors) + (process_id < (total_size % processors));
}

int get_full_matrix_index(int total_size, int local_index, int processes, int process_id) {
    int start = 0;
    for (size_t process = 0; process < process_id; process++)
    {
        start += get_chunk_size(total_size, processes, process);
    }

    return start + local_index;
}

void copy_chunk_to_main(double* chunk_buf, double* main_data, int main_size, int processes, int process_id) {
    int chunk_cols = get_chunk_size(NODES, processes, process_id);

    // Chunk pointers
    double* c_Bx = chunk_buf + 0 * chunk_cols;
    double* c_By = chunk_buf + 1 * chunk_cols;
    double* c_Bz = chunk_buf + 2 * chunk_cols;
    double* c_rho = chunk_buf + 3 * chunk_cols;
    double* c_Mx = chunk_buf + 4 * chunk_cols;
    double* c_My = chunk_buf + 5 * chunk_cols;
    double* c_Mz = chunk_buf + 6 * chunk_cols;
    double* c_E = chunk_buf + 7 * chunk_cols;

    // Main pointers
    double* m_Bx = main_data + 0 * NODES;
    double* m_By = main_data + 1 * NODES;
    double* m_Bz = main_data + 2 * NODES;
    double* m_rho = main_data + 3 * NODES;
    double* m_Mx = main_data + 4 * NODES;
    double* m_My = main_data + 5 * NODES;
    double* m_Mz = main_data + 6 * NODES;
    double* m_E = main_data + 7 * NODES;

    // print_matrix(main_data, NODES);
    // printf("-------------------\n");
    // print_matrix(chunk_buf, chunk_cols);

    // Get the column where the chunk starts
    int column_offset = get_full_matrix_index(NODES, 0, processes, process_id);

    for (size_t i = 0; i < chunk_cols; i++)
    {
        int real_i = i + column_offset;
        m_Bx[real_i] = c_Bx[i];
        m_By[real_i] = c_By[i];
        m_Bz[real_i] = c_Bz[i];
        m_rho[real_i] = c_rho[i];
        m_Mx[real_i] = c_Mx[i];
        m_My[real_i] = c_My[i];
        m_Mz[real_i] = c_Mz[i];
        m_E[real_i] = c_E[i];
...
void set_row_values(
    ...
    double* Bz,
    int processes, int process_id, int chunk_size) {
    double rho1 = 1.0;
    double Vx1 = 0.0;
    double Vy1 = 0.0;
    double Vz1 = 0.0;
    ...
    {
        size_t real_x_index = get_full_matrix_index(NODES, i, processes, process_id);
        double tanh_val = tanh(x[real_x_index] / W);
...
double* getU(
    ...
    int processes, int process_id, int local_data_size, int chunk_size) {
    
    int u_size = LOCAL_U_SIZE(local_data_size);
    int u_size_bytes = u_size * sizeof(double);
    double* U = malloc(u_size_bytes);
    memset(U, 0, u_size_bytes);

    double* u_Bx = U + 0 * local_data_size;
    double* u_By = U + 1 * local_data_size;
    double* u_Bz = U + 2 * local_data_size;
    double* u_rho = U + 3 * local_data_size;
    double* Mx = U + 4 * local_data_size;
    double* My = U + 5 * local_data_size;
    double* Mz = U + 6 * local_data_size;
    double* E = U + 7 * local_data_size;
    ...
    if(process_id == 0) {
        double* buf = (double*)malloc(u_size_bytes);
        MPI_Status status;
        for (size_t process = 0; process < processes - 1; process++)
        {
            MPI_Recv(buf, u_size, MPI_DOUBLE, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &status);
            copy_chunk_to_main(buf, U, u_size, processes, status.MPI_SOURCE);
        }
        free(buf);
    }
    else {
        MPI_Send(U, u_size, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
        free(U);
    }
...
int main(int argc, char** argv) {
    ...
    int chunk_size = get_chunk_size(NODES, processes, process_id);
    int local_data_size = process_id == 0 ? NODES : chunk_size;

    double* x = linspace(-1.0, 1.0, NODES);

    double* rho = malloc(local_data_size * sizeof(double));
    double* Vx = malloc(local_data_size * sizeof(double));
    double* Vy = malloc(local_data_size * sizeof(double));
    double* Vz = malloc(local_data_size * sizeof(double));
    double* p = malloc(local_data_size * sizeof(double));
    double* Bx = malloc(local_data_size * sizeof(double));
    double* By = malloc(local_data_size * sizeof(double));
    double* Bz = malloc(local_data_size * sizeof(double));

    set_row_values(x, rho, Vx, Vy, Vz, p, Bx, By, Bz, processes, process_id, chunk_size);
    double* U = getU(Bx, By, Bz, rho, p, Vx, Vy, Vz, processes, process_id, local_data_size, chunk_size);
